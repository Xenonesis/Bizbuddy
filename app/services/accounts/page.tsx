			...
			<section className="hero-sky-wash pt-12 sm:pt-16 pb-10 sm:pb-14 border-b border-[#dcdee0] dark:border-[#27272a]">
				<div className="max-w-[1280px] mx-auto px-4 sm:px-6 text-center space-y-4">
					...
					<p className="text-base text-[#60646c] dark:text-zinc-400">
						Never miss a tax deadline again. Professional chartered accountants
						handling EPF, TDS, Income Tax Returns, ROC compliance, and company
						registrations.
					</p>
				</div>
			</section>
			...
			<section className="max-w-[1280px] mx-auto px-4 sm:px-6 space-y-8">
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{ACCOUNT_SERVICES.map((svc) => (
						<div
							key={svc.id}
							className="card-surface bg-white dark:bg-[#14161b] border border-[#dcdee0] dark:border-[#27272a] rounded-xl p-5 sm:p-6 flex flex-col justify-between hover:border-[#171717] dark:hover:border-zinc-500 transition-all"
						>
							...
							<div className="pt-6 border-t border-[#f0f0f3] dark:border-[#27272a] mt-4 flex items-center justify-between gap-2">
								<div>
									...
									<div className="text-sm font-bold text-[#171717] dark:text-[#f4f4f5] font-mono">
										{svc.pricing}
									</div>
								</div>
								<button
					...
					<div className="bg-white dark:bg-[#14161b] rounded-xl border border-[#dcdee0] dark:border-[#27272a] w-full max-w-md p-6 space-y-4">
						...
						<ul className="space-y-2 text-sm text-[#171717] dark:text-[#f4f4f5]">
							{selectedDocService.documents.map((doc, idx) => (
								<li
									key={doc}
									className="flex items-center gap-2 p-2 rounded-md bg-[#fafafa] dark:bg-[#1e2026]"
								>
