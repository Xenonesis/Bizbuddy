			...
			<section className="hero-sky-wash pt-12 sm:pt-16 pb-10 sm:pb-14 border-b border-[#dcdee0] dark:border-[#27272a]">
				<div className="max-w-[1280px] mx-auto px-4 sm:px-6 text-center space-y-4">
					...
					<p className="text-base text-[#60646c] dark:text-zinc-400">
						Grab audience attention in the first 3 seconds. Viral captions,
						kinetic text animations, color grading, sound design, and realistic
						AI avatars.
					</p>
				</div>
			</section>
			...
			<section className="max-w-[1280px] mx-auto px-4 sm:px-6">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
					{EDITING_SERVICES.map((item) => (
						<div
							key={item.id}
							className="card-surface p-8 space-y-6 flex flex-col justify-between bg-white dark:bg-[#14161b] border border-[#dcdee0] dark:border-[#27272a] rounded-xl"
						>
							<div className="space-y-4">
								<div className="flex justify-between items-start border-b border-[#f0f0f3] dark:border-[#27272a] pb-3">
									<h3 className="text-xl font-semibold text-[#171717] dark:text-[#f4f4f5] flex items-center gap-2">
										<Video className="w-5 h-5 text-[#0d74ce] dark:text-[#38bdf8]" />
										{item.title}
									</h3>
									<span className="font-mono text-sm font-bold text-[#171717] dark:text-[#f4f4f5]">
										{item.price}
									</span>
								</div>
								...
								<div className="space-y-2">
									...
									<ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-[#171717] dark:text-[#f4f4f5]">
										{item.deliverables.map((del) => (
											<li
												key={del}
												className="flex items-center gap-1.5 bg-[#fafafa] dark:bg-[#0c0d10] p-2 rounded border border-[#f0f0f3] dark:border-[#27272a]"
											>
