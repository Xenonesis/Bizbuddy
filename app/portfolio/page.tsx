			...
			<section className="hero-sky-wash pt-12 sm:pt-16 pb-10 sm:pb-14 border-b border-[#dcdee0] dark:border-[#27272a]">
				<div className="max-w-[1280px] mx-auto px-4 sm:px-6 text-center space-y-4">
					...
					<p className="text-base text-[#60646c] dark:text-zinc-400">
						Explore how we help clinics, restaurants, real estate agencies, and
						D2C brands scale revenue.
					</p>
				</div>
			</section>
			...
			<section className="max-w-[1280px] mx-auto px-4 sm:px-6 space-y-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-8">
					{filtered.map((item) => (
						<div
							key={item.id}
							className="card-surface bg-white dark:bg-[#14161b] border border-[#dcdee0] dark:border-[#27272a] rounded-xl overflow-hidden group"
						>
							...
							<div className="p-6 space-y-3">
								<div className="text-xs font-mono text-[#0d74ce] dark:text-[#38bdf8] font-semibold">
									{item.client}
								</div>
								<h3 className="font-semibold text-lg text-[#171717] dark:text-[#f4f4f5]">
									{item.title}
								</h3>
								...
								<div className="pt-2 flex flex-wrap gap-1.5">
									{item.tags.map((tag) => (
										<span
											key={tag}
											className="text-[10px] bg-[#f0f0f3] dark:bg-[#27272a] text-[#171717] dark:text-[#f4f4f5] px-2 py-0.5 rounded font-mono"
										>
											#{tag}
										</span>
									))}
