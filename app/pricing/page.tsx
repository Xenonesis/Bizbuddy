                ...
                <ul className="space-y-2.5 text-sm">
                  {plan.features.map((feat) => (
                    <li key={feat} className="flex items-start gap-2.5">
                      ...
                      <span className={plan.popular ? "text-zinc-200" : "text-[#171717] dark:text-[#f4f4f5]"}>{feat}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-[#fafafa] dark:bg-[#0c0d10] border-b border-[#dcdee0] dark:border-[#27272a] text-xs font-mono uppercase text-[#60646c] dark:text-zinc-400">
                ...
                <th className="p-4 bg-zinc-100 dark:bg-zinc-800 font-bold text-black dark:text-white">Super (₹1,999)</th>
                <th className="p-4">Premium (₹2,499)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#f0f0f3] dark:divide-[#27272a] text-sm">
              {comparisonFeatures.map((row) => (
                <tr key={row.name} className="hover:bg-[#fafafa] dark:hover:bg-[#1e2026]">
                  <td className="p-4 font-medium text-[#171717] dark:text-[#f4f4f5]">{row.name}</td>
