...
const BorderGlow = ({
  ...
  const actualBg = backgroundColor === 'auto' ? (isDark ? '#14161b' : '#ffffff') : backgroundColor;
  const actualBorder = borderColor === 'auto' ? (isDark ? '#27272a' : '#dcdee0') : borderColor;
  const actualBlend = glowBlendMode === 'auto' ? (isDark ? 'plus-lighter' : 'normal') : glowBlendMode;
  ...
  return (
    <div
      ...
      style={{
        '--card-bg': actualBg,
        '--border-color': actualBorder,
        '--glow-blend-mode': actualBlend,
    ...
    >
      ...
      <div className="border-glow-inner" style={{ overflow: 'visible' }}>
        {children}
      </div>
    </div>
  );
